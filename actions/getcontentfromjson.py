...
class GetContentFromJsonConfig:
    ...
    def from_dict(data: dict[str, Any]) -> Result['GetContentFromJsonConfig', str]:
        ...
        errs = [err for err in [query_res.swap().default_value(None), output_name_res.swap().default_value(None), return_empty_result_res.swap().default_value(None)] if err is not None]
        match errs:
            case []:
